...
class ActionOrchestrator:
    ...
    def to_dict(self) -> dict[str, Any]:
        """Serialize the ActionOrchestrator state for cross-session persistence.

        Saves all registered actions with their states and results,
        execution log, topological levels, statistics, and configuration.
        The agent can resume execution exactly where it left off.

        Returns:
            A JSON-serializable dict of all orchestrator state.
        """
        return {
            "actions": [
                {
                    ...
                    "dependencies": [str(d) for d in a.dependencies],
                    "retry_policy": {
                        "max_retries": a.retry_policy.max_retries,
                        "base_delay": a.retry_policy.base_delay,
                        "backoff_factor": a.retry_policy.backoff_factor,
                        "max_delay": a.retry_policy.max_delay,
                    },
                    "tags": list(a.tags),
            ...
            "results": {
                str(aid): [
                    {
                        ...
                        "success": r.success,
                        "output": None,  # output is non-serializable
                        "error": r.error,
                ...
                for aid, results in self._results.items()
            },
            "completed": [str(aid) for aid in self._completed],
            "topological_levels": {
                str(aid): level
                for aid, level in self._topological_levels.items()
            },
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> ActionOrchestrator:
        """Deserialize ActionOrchestrator state.

        Reconstructs all actions with their states, results, topological
        levels, and statistics from saved data.

        Args:
            data: A dict produced by to_dict().

        Returns:
            A reconstructed ActionOrchestrator with full execution state.
        """
        from uuid import UUID
        from isonome.praxis.orchestrator import Action, ActionRisk, ActionState, ExecutionResult, RetryPolicy

        # Parse retry policy if present
        retry_data = data.get("default_retry_policy", {})
        ...
        from isonome.praxis.orchestrator import ActionOrchestrator as AO

        orchestrator = AO.__new__(AO)
        orchestrator._actions = {}
        orchestrator._states = {}
        orchestrator._results = {}
        orchestrator._completed = set()
        orchestrator._topological_levels = {}
        orchestrator._max_parallel = data.get("max_parallel", 8)
        orchestrator._default_retry = default_retry
        orchestrator._engine = None
        orchestrator._confidence_calibrator = None
        orchestrator._total_executed = 0
        orchestrator._total_completed = 0
        orchestrator._total_failed = 0
        orchestrator._total_retried = 0
        orchestrator._total_blocked = 0
        orchestrator._batch_count = 0
        orchestrator._execution_log: list[dict[str, Any]] = []
        orchestrator._tension_override: dict[TensionID, float] = {}

        # Restore actions
        saved_actions = data.get("actions", [])
        for a_data in saved_actions:
            rp_data = a_data.get("retry_policy", {})
            rp = RetryPolicy(
                max_retries=rp_data.get("max_retries", 3),
                base_delay=rp_data.get("base_delay", 1.0),
                backoff_factor=rp_data.get("backoff_factor", 2.0),
                max_delay=rp_data.get("max_delay", 300.0),
            )
            action = Action(
                id=UUID(a_data["id"]),
                description=a_data["description"],
                tool_name=a_data["tool_name"],
                params=dict(a_data.get("params", {})),
                risk=ActionRisk[a_data.get("risk", "LOW")],
                preconditions=tuple(a_data.get("preconditions", [])),
                expected_outcome=a_data.get("expected_outcome", ""),
                dependencies=tuple(UUID(d) for d in a_data.get("dependencies", [])),
                retry_policy=rp,
                tags=tuple(a_data.get("tags", [])),
                metadata=dict(a_data.get("metadata", {})),
                confidence_required=float(a_data.get("confidence_required", 0.0)),
            )
            orchestrator._actions[action.id] = action

        # Restore states
        saved_states = data.get("states", {})
        for aid_str, state_name in saved_states.items():
            try:
                state = ActionState[state_name]
            except KeyError:
                state = ActionState.PENDING
            orchestrator._states[UUID(aid_str)] = state

        # Restore results
        saved_results = data.get("results", {})
        for aid_str, results_list in saved_results.items():
            aid = UUID(aid_str)
            orchestrator._results[aid] = []
            for r_data in results_list:
                result = ExecutionResult(
                    action_id=UUID(r_data["action_id"]),
                    success=bool(r_data["success"]),
                    output=None,
                    error=r_data.get("error", ""),
                    duration_ms=float(r_data.get("duration_ms", 0.0)),
                    validation_passed=bool(r_data.get("validation_passed", False)),
                    validation_score=float(r_data.get("validation_score", 0.0)),
                    attempt=int(r_data.get("attempt", 0)),
                    timestamp=float(r_data.get("timestamp", 0.0)),
                )
                orchestrator._results[aid].append(result)

        # Restore completed set
        saved_completed = data.get("completed", [])
        for aid_str in saved_completed:
            orchestrator._completed.add(UUID(aid_str))

        # Restore topological levels
        saved_levels = data.get("topological_levels", {})
        for aid_str, level in saved_levels.items():
            orchestrator._topological_levels[UUID(aid_str)] = int(level)

        # Restore counters
        orchestrator._total_executed = int(data.get("total_executed", 0))
        orchestrator._total_completed = int(data.get("total_completed", 0))
        orchestrator._total_failed = int(data.get("total_failed", 0))
        orchestrator._total_retried = int(data.get("total_retried", 0))
        orchestrator._total_blocked = int(data.get("total_blocked", 0))
        orchestrator._batch_count = int(data.get("batch_count", 0))
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        for aid, result_list in self._results.items():
            serialized_results = []
    ...
    def from_dict(cls, data: dict[str, Any], **kwargs: Any) -> ActionOrchestrator:
        ...
        # Rebuild results
        for aid_str, result_list in data.get("results", {}).items():
            aid = UUID(aid_str)
            restored_results = []
            for r_data in result_list:
                restored_results.append(ExecutionResult(
                    action_id=UUID(r_data["action_id"]),
                    success=r_data["success"],
                    error=r_data.get("error", ""),
                    duration_ms=r_data.get("duration_ms", 0.0),
                    validation_passed=r_data.get("validation_passed", False),
                    validation_score=r_data.get("validation_score", 0.0),
                    attempt=r_data.get("attempt", 0),
                    timestamp=r_data.get("timestamp", 0.0),
                ))
            orch._results[aid] = restored_results

        # Rebuild default retry policy
        retry_data = data.get("default_retry_policy", {})
        orch._default_retry = RetryPolicy(
            max_retries=retry_data.get("max_retries", 3),
            base_delay=retry_data.get("base_delay", 1.0),
            backoff_factor=retry_data.get("backoff_factor", 2.0),
            max_delay=retry_data.get("max_delay", 300.0),
