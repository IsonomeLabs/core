...
class BasePillar(PillarProtocol, abc.ABC):
	...
	def __init__(
		...
		*,
		...
		# ── Momentum-modulated stress feedback (iter-029) ────────
		# When velocity tracking is active, the stress restoring force
		# is modulated by momentum: drifting axes get a stronger pull,
		# approaching axes get a weaker pull (let them coast).
		if momentum_drifting_multiplier <= 0:
			raise ValueError(
				"momentum_drifting_multiplier must be positive, "
				f"got {momentum_drifting_multiplier}"
			)
		if momentum_approaching_multiplier <= 0:
			raise ValueError(
				"momentum_approaching_multiplier must be positive, "
				f"got {momentum_approaching_multiplier}"
			)
		self._momentum_drifting_multiplier: float = momentum_drifting_multiplier
		self._momentum_approaching_multiplier: float = momentum_approaching_multiplier
		self._momentum_stress_modulation_enabled: bool = momentum_stress_modulation_enabled
		self._momentum_stress_modulations: int = 0
	...
	def process_queued(self) -> None:
		...
		if self._equilibrium_view is not None:
	...
	def _emit_stress_feedback(self) -> None:
		...
		- 0.3 < stress_level <= 0.5: mild feedback (sigma x 0.05 toward default)
		- stress_level > 0.5: moderate feedback (sigma x 0.10 toward default)

		When velocity tracking is active (iter-029), the restoring force
		is momentum-modulated:
		- Drifting axis (momentum < 0): boosted by drifting_multiplier
		  -- axis is moving away, so pull harder
		- Approaching axis (momentum > 0): dampened by approaching_multiplier
		  -- axis is heading home, let it coast
		- Zero momentum / no velocity data: unchanged (backward-compatible)
		...
		# ── Momentum modulation (iter-029) ────────────────────
		# Apply momentum-aware modulation when velocity tracking is
		# active and the feature is enabled.
		momentum_suffix = ""
		if (
			self._momentum_stress_modulation_enabled
			and self._equilibrium_view is not None
		):
			momentum_scores = self._equilibrium_view.momentum_scores
			if momentum_scores:  # velocity tracking is active
				momentum = momentum_scores.get(max_axis, 0.0)
				if momentum < 0:
					# Drifting away from default -- boost restoring force
					multiplier = self._momentum_drifting_multiplier
					signal_val *= multiplier
					momentum_suffix = (
						f", momentum-boosted x{multiplier:.2f}"
					)
					self._momentum_stress_modulations += 1
				elif momentum > 0:
					# Approaching default -- weaken restoring force
					multiplier = self._momentum_approaching_multiplier
					signal_val *= multiplier
					momentum_suffix = (
						f", momentum-dampened x{multiplier:.2f}"
					)
					self._momentum_stress_modulations += 1
				# momentum == 0: no modulation (stationary axis)

		# Clamp signal to valid range after modulation
		clamped_signal = max(-1.0, min(1.0, signal_val))
