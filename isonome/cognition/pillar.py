...
class CognitionPillar(BasePillar):
    ...
    def _on_equilibrium_sync(self, view) -> None:
        ...
        Applies the view's own_axes positions to the reasoning engine
        and pushes calibration state to the attention system — the
        same logic as update_tension_profile() but driven by pull
        instead of push.
        """
        if self.reasoning is not None:
            self.reasoning.set_tension_profile(view.all_positions)
        ...
        # reduce reasoning depth to match
        if view.is_axis_oscillating("explore_exploit"):
            logger.debug(
                f"{self.name}: explore_exploit oscillating — "
