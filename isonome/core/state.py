...
class TopologyVectorState(BaseModel):
    """Serializable topology vector state for inter-layer communication.

    Wraps the 32-D topology features and SHA-256 topology hash produced by
    ``MorphologyAnalyzer`` so they can flow through the agent's state
    pipeline and be persisted/serialized via Pydantic.
    """

    model_config = ConfigDict(arbitrary_types_allowed=True)

    features: torch.Tensor  # shape: [32]
    topology_hash: str = ""
    ...
    def _validate_tensor(cls, v: Any) -> torch.Tensor:
        t = torch.as_tensor(v)
        if t.shape != (32,):
            raise ValueError(f"TopologyVectorState.features must have shape (32,), got {t.shape}")
        return t
    ...
    @classmethod
    def from_topology_vector(cls, tv: "TopologyVector") -> "TopologyVectorState":
        """Create from a ``isonome.utils.morphology.TopologyVector``."""
        # Late import to avoid circular dependency
        return cls(features=tv.features.clone(), topology_hash=tv.topology_hash)
